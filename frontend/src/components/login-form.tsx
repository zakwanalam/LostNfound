...
export function LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.post("http://localhost:5086/api/auth/login", {
        email,
        password
      ...
      window.FB.login(
        (response) => {
          if (response.authResponse) {
            ...
            const login = async () => {
              await axios.post("http://localhost:5086/api/auth/loginFacebook", { accessToken }, { withCredentials: true })
                .then((res) => {
  ...
  return (
    <>
      ...
      <div className="bg-[url('/lost_found.jpg')] bg-no-repeat w-full min-h-screen flex justify-center items-center">
        <div className={cn("max-w-sm sm:max-w-md opacity-95", className)} {...props}>
          <Card>
              ...
              <form onSubmit={handleSubmit}>
                <div className="grid gap-6">
                  <div className="flex flex-col gap-4">
                    <Button onClick={loginWihFacebook} variant="outline" className="w-full">
